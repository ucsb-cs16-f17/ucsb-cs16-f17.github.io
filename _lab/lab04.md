...
The program takes 3 arguments at the command line: an integer, a character, and another integer. The character can be one of only 3 types: '+', '*', or '%'. The program then returns either the sum, the product, or the modulo of the 2 integers.

If the number of arguments is not exactly three (other than the program name itself), a "Usage" message is printed (see the sample runs below). <b>Hint:</b> you may want to use <code>cerr</code> instead of <code>cout</code> to output the usage message.
